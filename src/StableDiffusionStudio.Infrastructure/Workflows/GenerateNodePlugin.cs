...
/// <summary>
/// Core generation node. When image input is connected, behaves as img2img.
/// When no image input, behaves as txt2img.
/// </summary>
public class GenerateNodePlugin : IWorkflowNodePlugin
{
    public string PluginId => "core.generate";
    public string DisplayName => "Generate";
    public string Category => "Generation";
    public string Description => "Text-to-image generation. Connect an image input for img2img mode.";
    public string Icon => "Icons.Material.Filled.AutoAwesome";
        ...
        CancellationToken ct = default)
    {
        ...
        byte[]? initImage = inputs.TryGetValue("image", out var imageData) ? imageData.ImageBytes : null;
        ...
        var request = new InferenceRequest(
            ...
            parameters.Width,
            parameters.Height,
            1, // Single image per workflow step
            parameters.ClipSkip,
            parameters.Eta,
            initImage,
            initImage is not null ? parameters.DenoisingStrength : 1.0,
        ...
        var progress = new Progress<InferenceProgress>(p =>
        {
            context.Progress.Report(new WorkflowStepProgress(
                context.NodeId, p.Step, p.TotalSteps, "Generating"));
        });

        var result = await context.InferenceBackend.GenerateAsync(request, progress, ct);

        if (result.Images.Count == 0)
            throw new InvalidOperationException("Generation produced no images.");
    ...
    private static GenerationParameters DeserializeParameters(string? config)
    {
        if (string.IsNullOrWhiteSpace(config))
            throw new InvalidOperationException("Generate node requires parameters configuration.");
